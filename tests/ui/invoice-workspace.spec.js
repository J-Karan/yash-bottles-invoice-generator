...
test('login screen renders and invalid login reports an error', async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole('button', { name: 'Version 0.3.2' })).toBeVisible()
  await page.getByRole('button', { name: 'Version 0.3.2' }).click()
  await expect(page.getByRole('heading', { name: 'Update history' })).toBeVisible()
  await expect(page.getByText('Version 0.3.2')).toBeVisible()
...
test('invoice workspace supports core interactions', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('.workspace-version')).toHaveText('Version 0.3.2')
