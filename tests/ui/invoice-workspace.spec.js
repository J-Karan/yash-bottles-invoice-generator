...
test('login screen renders and invalid login reports an error', async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole('button', { name: 'Version 0.2.1' })).toBeVisible()
  await page.getByRole('button', { name: 'Version 0.2.1' }).click()
...
test('invoice workspace supports core interactions', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('.workspace-version')).toHaveText('Version 0.2.1')
